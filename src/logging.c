...
static const ColorCombination log_level_color_combinations[] = {
    ...
    [LOG_LEVEL_TIME]  = { .fg = TC_BRIGHT_MAGENTA, .bg = TC_BRIGHT_MAGENTA },
    [LOG_LEVEL_WARN]  = { .fg = TC_YELLOW, .bg = TC_YELLOW },
    [LOG_LEVEL_TRACE] = { .fg = TC_WHITE, .bg = TC_WHITE },
};
...
void GDF_FlushLogBuffer()
{
    ...
    for (u32 i = 0; i < next_free_entry; i++)
    {
        ...
        snprintf(write_ptr, MAX_FORMATTED_ENTRY_LEN,
            "\x1B[%dm\x1B[%dm[%s @ %02d:%02d:%02d.%03d] %s %s\n", cc.fg, cc.bg, entry->thread_name,
            entry->time.hour, entry->time.minute, entry->time.second, entry->time.milli,
            level_strings[entry->level], entry->message);
...
{
    ThreadLoggingInfo info = { .thread_name = thread_name };
