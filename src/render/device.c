...
{
    VkFormat depth_formats[] = { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT,
...
void __get_queue_indices(GDF_VkRenderContext* vk_ctx, VkPhysicalDevice physical_device,
    GDF_VkPhysicalDeviceQueues* queues)
{
    ...
    for (u32 i = 0; i < queue_family_count; i++)
    {
        ...
        VK_ASSERT(vkGetPhysicalDeviceSurfaceSupportKHR(
            physical_device, i, vk_ctx->surface, &supports_present));
        if (supports_present)
        {
