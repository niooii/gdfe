...
)
{
    ...
    VK_RETURN_FALSE_ASSERT(
        vkAllocateMemory(device->handle, &alloc_info, GDFE_INTERNAL_VK_CTX->device.allocator, &out_buf->memory)
    );
    VK_RETURN_FALSE_ASSERT(
...
)
{
    ...
    VK_RETURN_FALSE_ASSERT(
        vkMapMemory(GDFE_INTERNAL_VK_CTX->device.handle, out_uniform_buf->buffer.memory, 0, size, 0, &out_uniform_buf->mapped_data)
    );
    return GDF_TRUE;
}
...
)
{
    vkFreeCommandBuffers(
        GDFE_INTERNAL_VK_CTX->device.handle,
        GDFE_INTERNAL_VK_CTX->transient_command_pool,
        1,
        command_buf
    );
}