...
void gdfe_get_surface_capabilities(VkPhysicalDevice physical_device, VkSurfaceKHR surface,
    GDF_VkPhysicalDeviceSwapchainSupport* out_support_info)
{
    VK_ASSERT(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        physical_device, surface, &out_support_info->capabilities));
}

void gdfe_query_sc_info(VkPhysicalDevice physical_device, VkSurfaceKHR surface,
    GDF_VkPhysicalDeviceSwapchainSupport* out_support_info)
{
    gdfe_get_surface_capabilities(physical_device, surface, out_support_info);

    VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(
        physical_device, surface, &out_support_info->format_count, NULL));
    ...
    if (out_support_info->format_count != 0)
    {
        ...
        {
            out_support_info->formats = GDF_Malloc(
                sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, GDF_MEMTAG_RENDERER);
        }
        VK_ASSERT(vkGetPhysicalDeviceSurfaceFormatsKHR(
    ...
    if (out_support_info->present_mode_count != 0)
    {
        if (!out_support_info->present_modes)
        {
            out_support_info->present_modes =
                GDF_Malloc(sizeof(VkPresentModeKHR) * out_support_info->present_mode_count,
                    GDF_MEMTAG_RENDERER);
        }
...
GDF_BOOL gdfe_swapchain_init(GDF_VkRenderContext* vk_ctx, u16 w, u16 h)
{
    ...
    VkSwapchainCreateInfoKHR sc_create_info = { .sType =
    ...
    VK_ASSERT(vkCreateSwapchainKHR(vk_ctx->device.handle, &sc_create_info, vk_ctx->device.allocator,
    ...
    VK_ASSERT(vkGetSwapchainImagesKHR(
        vk_ctx->device.handle, vk_ctx->swapchain.handle, &image_count, vk_ctx->swapchain.images));
    ...
    for (u32 i = 0; i < image_count; i++)
    {
        ...
        VK_ASSERT(vkCreateImageView(vk_ctx->device.handle, &image_view_info,
            vk_ctx->device.allocator, &vk_ctx->swapchain.image_views[i]));
    }
...
void gdfe_swapchain_destroy(GDF_VkRenderContext* vk_ctx)
{
    ...
    {
        vkDestroyImageView(device, vk_ctx->swapchain.image_views[i], allocator);
    }
