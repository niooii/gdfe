...
GDF_Thread GDF_CreateThread(unsigned long thread_fn(void*), void* args)
{
    ...
    thread->thread_handle     = CreateThread(NULL, 0, thread_fn, args, creation_flags, &thread->id);

    return thread;
}

void GDF_DestroyThread(GDF_Thread thread) { GDF_Free(thread); }

u32 GDF_GetCurrentThreadId() { return GetCurrentThreadId(); }

void GDF_JoinThread(GDF_Thread thread) { WaitForSingleObject(thread->thread_handle, INFINITE); }

void GDF_ThreadSleep(u64 ms) { Sleep(ms); }
...
GDF_Semaphore GDF_GetSemaphore(const char* name)
{
    ...
    HANDLE handle = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, namebuf);
