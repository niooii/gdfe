    ...
    case WM_SYSKEYUP:
        {
            ...
            // Keypress only determines if _any_ alt/ctrl/shift key is pressed. Determine which one
            // if so.
            if (w_param == VK_MENU)
            {
...
GDF_Window GDF_CreateWindow(i16 x_, i16 y_, i16 w, i16 h, const char* title)
{
    ...
    HWND handle = CreateWindowExA(window_ex_style, win_class_name, title, window_style, window_x,
        window_y, window_width, window_height, 0, 0, class_h_instance, 0);
    ...
    // Show the window
    GDF_BOOL should_activate =
        GDF_TRUE; // TODO! if the window should not accept input, this should be GDF_FALSE.
