...
typedef struct GDF_AppState {
    ...
    /// @note Setting this to \code GDF_FALSE\endcode will cause the application to exit after the
    /// current iteration.
    GDF_BOOL alive;
} GDF_AppState;
...
typedef struct GDF_RenderCallbacks {
    ...
    (const GDF_VkRenderContext* vulkan_ctx, const GDF_AppState* app_state, void* state);
    ...
    GDF_BOOL(*on_render)
    (const GDF_VkRenderContext* vulkan_ctx, GDF_RENDER_MODE mode, const GDF_AppState* app_state,
     void* state);
...
typedef struct GDF_AppCallbacks {
    ...
    /// the input subsystem.
    GDF_BOOL (*on_loop)(const GDF_AppState* app_state, f64 delta_time, void* state);
