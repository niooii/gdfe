...
GDF_Set GDF_SetCreateFull(u32 stride, u32 (*hash_func)(const u8* data, u32 len), GDF_BOOL is_string,
                          u32 initial_capacity);

    #define GDF_SetCreate(type, is_string) GDF_SetCreateFull(sizeof(type), NULL, is_string, 32)
...
// Returns a pointer to the inserted value (owned by set) on success or if it already existed, and
// NULL on an error. Returns NULL if the requested value is NULL. already_existed may be NULL.
