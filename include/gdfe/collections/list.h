...
enum { LIST_CAPACITY, LIST_LENGTH, LIST_STRIDE, LIST_FIELD_LENGTH };
...
// #define GDF_LIST_Contains
    ...
    /// @note Storing multiple pointers of the same list is unsafe. The pointer may be reassigned by
    /// the \code GDF_ListPush*\endcode macros.
    #define GDF_ListCreate(type) gdfe_list_create(LIST_DEFAULT_CAPACITY, sizeof(type))

    #define GDF_ListReserve(type, capacity) gdfe_list_create(capacity, sizeof(type))
    ...
    #define GDF_ListPop(list, out_val_p) gdfe_list_pop(list, out_val_p)
    ...
    #define GDF_ListRemove(list, index, out_val_p) gdfe_list_remove_at(list, index, out_val_p)

    #define GDF_ListClear(list) gdfe_list_field_set(list, LIST_LENGTH, 0)

    #define GDF_ListCapacity(list) gdfe_list_field_get(list, LIST_CAPACITY)

    #define GDF_ListLen(list) gdfe_list_field_get(list, LIST_LENGTH)

    #define GDF_ListStride(list) gdfe_list_field_get(list, LIST_STRIDE)

    #define GDF_ListSetLen(list, value) gdfe_list_field_set(list, LIST_LENGTH, value)
