...
GDF_Set __set_create(u32 stride, u32 (*hash_func)(const u8* data, u32 len), bool is_string, u32 initial_capacity);

#define GDF_SetCreate(type, is_string) \
    __set_create(sizeof(type), NULL, is_string, 32)

#define GDF_SetReserve(type, is_string, initial_capacity) \
    __set_create(sizeof(type), NULL, is_string, initial_capacity)

#define GDF_SetWithHasher(type, hash_func, is_string) \
    __set_create(sizeof(type), hash_func, is_string, 32)

#define GDF_SetReserveWithHasher(type, hash_func, is_string, initial_capacity) \
    __set_create(sizeof(type), hash_func, is_string, 32, initial_capacity)
...
bool GDF_SetContains(GDF_Set set, void* value);
...
// True is returned on success, false is returned if the value could not be found
